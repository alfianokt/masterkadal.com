<script>
  ...
  async function spam (target) {
    try {
      const request = await axios.get(`https://pranks.masterkadal.com/api/oyo/${target}`)
      const status = request.data.status == "correct" ? 'Successfully ✅' : 'Failed ❌'
    ...
    <div class="column col-12">
      <div class="col-md-12 col-6">
        <div class="input-group">
          ...
          <input class="form-input" type="number" id="input-target" placeholder="085..." class:disabled={auto}>
